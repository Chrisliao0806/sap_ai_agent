...
- 考慮產品的技術先進性和未來適用性
- 評估供應商的可靠性和服務品質
- 只要使用者有提出價格上面的需求，請前萬不要推薦比他價格更高的設備了，因為他就是買不起。
...
- 自然地詢問缺少的資訊，不要太正式""",
    ...
    @staticmethod
    def get_extract_product_from_recommendation_prompt():
        """從推薦中提取產品資訊提示"""
        return ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """你是一個專業的產品資訊提取分析師。請從 LLM 推薦文字中提取推薦的具體產品資訊。

分析任務：
1. 仔細分析推薦文字，找出被推薦的具體產品
2. 提取產品名稱、價格、供應商等關鍵資訊
3. 如果推薦中提到了採購歷史中的產品，優先匹配歷史產品
4. 如果沒有歷史匹配，則基於推薦內容創建產品資訊
...
{{
    "recommended_product": {{
        ...
        "unit_price": 價格數字,
        "supplier": "供應商名稱",
        "source": "history"或"recommendation"
    }},
    "confidence_score": 0.0-1.0,
    "extraction_notes": "提取過程說明"
}}
```
...
- 準確提取產品名稱（如：14吋、16吋）
- 價格必須是整數
- 如果推薦中沒有明確價格，基於上下文推斷合理價格
- source 欄位用於標識產品來源（採購歷史或推薦內容）
- confidence_score 表示提取準確度""",
                ),
                (
                    "human",
                    """
